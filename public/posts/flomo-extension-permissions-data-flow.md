...
coverWidth: 1200
coverHeight: 900
excerpt: 按扩展 1.30.0 源码逐项说明 storage、tabs、alarms、identity 以及全网页选中文本脚本的用途，并区分本地草稿、扩展账户与 flomo 账户的数据流。
---

安装浏览器扩展时，只看到一组权限名称，很难判断它们与实际功能有什么关系。本文不使用“安全可靠”之类无法由代码单独证明的结论，而是依据 Flomo Extension 1.30.0 源码，把每项权限对应到具体操作，并说明当前实现的边界。

> 核验结论适用于本文检查的 1.30.0 源码，不等同于对 Chrome 商店安装包的独立安全审计。商店版本、依赖和后续代码都可能变化，安装时仍应以浏览器展示的权限与本站[隐私政策](/privacy)为准。
...
| `alarms` | 设计上用于每 5 分钟检查一次 flomo 标签页 | 安装回调尝试创建任务；1.30.0 存在异步判断缺陷，不能确认任务一定建立 |
...
但 1.30.0 的安装回调有一个重要实现问题：代码调用异步的 `browser.alarms.get()` 后没有等待结果，就直接判断返回值。使用 Promise 形式的浏览器 API 时，这个返回值本身是一个 Promise 对象，后续“没有任务就创建”的分支不能按预期执行。因此，仅凭当前源码不能声称周期任务已经成功建立。
...
- 后台任务如果存在，每 5 分钟可能主动打开 flomo 页面；1.30.0 的任务创建路径本身有缺陷，且当前没有可见开关；
...
本文由 Flomo Extension 项目于 2026 年 8 月 1 日依据应用源码核对，目标包版本为 `1.30.0`。检查范围包括扩展清单、后台任务、普通网页内容脚本、flomo 页面内容脚本、弹窗编辑器、登录接口调用与本地存储封装。本文描述“代码会做什么”，不代替第三方安全测试，也不对 flomo 官方页面的内部实现作推断。
