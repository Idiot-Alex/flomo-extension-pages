...
coverWidth: 1200
coverHeight: 900
excerpt: 按扩展 1.20.0 源码逐项说明 storage、tabs、alarms、identity 以及全网页选中文本脚本的用途，并区分本地草稿、扩展账户与 flomo 账户的数据流。
---

安装浏览器扩展时，只看到一组权限名称，很难判断它们与实际功能有什么关系。本文不使用“安全可靠”之类无法由代码单独证明的结论，而是依据 Flomo Extension 1.20.0 源码，把每项权限对应到具体操作，并说明当前实现的边界。

> 核验结论适用于本文检查的 1.20.0 源码，不等同于对 Chrome 商店安装包的独立安全审计。商店版本、依赖和后续代码都可能变化，安装时仍应以浏览器展示的权限与本站[隐私政策](/privacy)为准。
